...
class Barplot extends Plot { 
  ...
  public void drawAxes() {  
    ...
    textAlign(RIGHT);
    text("100%", tlX+offset-115, tlY+offset-10,100,50);
    ...
    textAlign(CENTER);
    pushMatrix();
    translate(tlX+offset-70,tlY+h-offset-yrange/2);
    rotate(1.5*PI);
    text(yLabel, 0,0);
    popMatrix();
  ...
  public void drawData() {
    ...
    for (int i=0;i<nBar;i++) {
      ...
      // fucked up labelling
      if (unique[i].equals("Europe")) { label="EU"; }
      else if (unique[i].equals("Africa")) { label="AF"; }
      else if (unique[i].equals("South America")) { label="SA"; }
      else if (unique[i].equals("Central America")) { label="CA"; }
      else if (unique[i].equals("North America")) { label="NA"; }
      else if (unique[i].equals("Oceania")) { label="OC"; }
      else if (unique[i].equals("Asia")) { label="AS"; }
      else { label="unknown"; }
      textSize(20);
      textAlign(CENTER);
      stroke(datac);
